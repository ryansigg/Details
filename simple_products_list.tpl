{*
$Id: simple_products_list.tpl,v 1.3.2.3.2.1 2012/08/07 09:50:07 tito Exp $
...
{if $products_matrix}
  ...
  <table cellspacing="3" class="products products-table simple-products-table width-100" summary="{$lng.lbl_products_list|escape}">
    ...
    {foreach from=$products_matrix item=row name=products_matrix}
          ...
          {if $product}
            ...
            <td{interline name=products additional_class="product-cell"}>
              <div class="image">
                ...
                <a href="product.php?productid={$product.productid}"{if $open_new_window eq 'Y'} target="_blank"{/if}>
                  {if $active_modules.On_Sale}
                    {include file="modules/On_Sale/on_sale_icon_modules_products.tpl" product=$product module="simple_products_list"}
                  {else}
      ...
      <tr{interline name=products_matrix additional_class="product-desinger-row"}>
      	{foreach from=$row item=product name=products}
      		{if $product}
